...
import java.util.*;
import javax.swing.*;
import mvias.ArbolMVias.Nodo;
/**
 *
 * @author THIAGUITO-PC
 */
public class Grafica   extends JPanel {
     private ArbolMVias miArbol;
     ...
     * dibujar.
     */
    public Grafica(ArbolMVias miArbol) 
    {
    ...
    private Dimension calcularTamañoSubarbol(Nodo n) 
    {
          ...
          Dimension ld = calcularTamañoSubarbol(n.hijo[0]);
          Dimension rd = calcularTamañoSubarbol(n.hijo[2]);//2
    ...
    *////PASOS PARA DIBUJAR LOS  CUADRITOS
    ...
    {      if (n == null) 
          return;      
      Dimension ld = (Dimension) subtreeSizes.get(n.hijo[0]);
      if (ld == null) 
          ld = empty;      
      Dimension rd = (Dimension) subtreeSizes.get(n.hijo[2]);
      if (rd == null) 
          rd = empty;      
      int center = 0;      
      ...
      if (n.dato[1]!=-9999)//cuande este vacio el hijo derecho
      {
      int width = fm.stringWidth(n.dato[0]+"  "+n.dato[1]+" "); //carga el tamaño del cuadrito
       posicionNodos.put(n,new Rectangle(center - width/2 - 3, top, width + 6, fm.getHeight()));
      }
      else
      {      int width = fm.stringWidth(n.hijo[0]+""+""); //carga el tamaño del cuadrito
       posicionNodos.put(n,new Rectangle(center - width/2 - 3, top, width + 6, fm.getHeight()));
      }      
      calcularPosicion(n.hijo[0], Integer.MAX_VALUE, center - child2child-25, top + fm.getHeight() + parent2child);// Coloca las posiciones de los cuadritos
      calcularPosicion(n.hijo[2], center + child2child+25, Integer.MAX_VALUE, top + fm.getHeight() + parent2child);
      calcularPosicion(n.hijo[1], center-15 , Integer.MAX_VALUE, top + fm.getHeight() + parent2child);// Hijo Medio
    }
    ...
    {if (n == null) 
     ...
     g.draw(r);
     if (n.dato[1]!=-9999)//cuando el hijo derecho este vacio
     {
     g.drawString(""+n.dato[0]+" | "+n.dato[1]+"", r.x + 3, r.y + yoffs);// carga los datos en un cuadro
     }
     else
     { g.drawString(""+n.dato[0]+" | "+"", r.x + 3, r.y + yoffs);// carga los datos en un cuadro
     }   
     if (puntox != Integer.MAX_VALUE)
       
     g.drawLine(puntox, puntoy, (int)(r.x + r.width/2), r.y);
     
     dibujarArbol(g, n.hijo[0], (int)(r.x + r.width-r.width), r.y + r.height, yoffs); //calcula de qe posicion se va a partir la Linea
     dibujarArbol(g, n.hijo[2], (int)(r.x + r.width), r.y + r.height, yoffs);
     dibujarArbol(g, n.hijo[1], (int)(r.x + r.width/2), r.y + r.height, yoffs);  //Hijo Medio 
   }
